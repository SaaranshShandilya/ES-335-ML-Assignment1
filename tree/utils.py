...
def change_intervals(y:pd.Series):
    prev = y[0]
    first_dropped = y.iloc[1:]
    index = 0
    split_points = []
    for ele in first_dropped:
        if prev == ele:
            index = index + 1
        else:
            prev = ele
...
def information_gain(Y: pd.Series,Y_low: pd.Series, Y_high:pd.Series, criterion: str) -> float:
    """
